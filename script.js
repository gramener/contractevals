...
function renderTable(data) {
  ...
  let fuse = new Fuse(data, {
    keys: ["Signatory Name", "Signatory Location"],
    threshold: 0.4,
  });
  ...
  const tableTemplate = (data) => html`
    <input type="text" class="form-control" placeholder="Search signatories or locations" @input=${searchData} />
          ...
          ${data.map(
            (row, i) => html`
              <tr data-id="${i}" @click=${() => showContractDetails(row)}>
                <td>
                  <a href="${row["eLIB link"]}" target="_blank" title="ISBN: ${row["ISBN"]}"
                    >${row["Signatory Name"]}</a
                  >
                  <div class="small">${row["Signatory Location"]}</div>
                </td>
                <td class="text-end">${formatDate(row["Agreement Date"])}</td>
                <td class="text-center" title="${row["Is agreement fully signed by all parties?"]}">
                  ${row["signed"]}
                </td>
